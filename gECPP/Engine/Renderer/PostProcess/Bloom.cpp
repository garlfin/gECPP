...
namespace gE::DefaultPipeline
{
	...
	void Bloom::RenderPass(Target2D& t, GL::Texture2D& in, GL::Texture2D& out)
	{
		...
		_shader.SetUniform(1, out.Use(1));
		...
		for(u8 i = 1; i < mipCount; i++)
		{
			...
			_shader.SetUniform(2, settings);
			out.Bind(0, GL_WRITE_ONLY, i);
		...
		for(u8 i = 1; i < mipCount; i++)
		{
			...
			settings.z = mip == 0 ? (float) BloomStage::UpsampleComposite : (float) BloomStage::Upsample;
			settings.w = mip + 1.f;

			_shader.SetUniform(2, settings);
			out.Bind(0, GL_WRITE_ONLY, mip);
