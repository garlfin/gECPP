...
namespace gE
{
	Camera::Camera(Entity* p, API::TextureSize2D size, IRenderTarget& t, const ICameraSettings& s, ComponentManager<Camera>* m) :
		Component(p, m),
		_settings(s), _target(t), _viewportSize(size)
	{
	}

	void Camera::GetGLCamera(API::Camera& cam)
	{
	...
	void PerspectiveCamera::GetGLCamera(API::Camera& camera)
	{
	...
	void Camera2D::GetGLCamera(API::Camera& camera)
	{
	...
	void Camera3D::GetGLCamera(API::Camera& cam)
	{
	...
	CameraCubemap::CameraCubemap(Entity* p, TARGET_TYPE& t, const CameraSettings1D& s, ComponentManager<Camera>* m) :
		Camera(p, API::TextureSize2D(s.Size), t, s, m)
	{
	}
	...
	void CameraCubemap::GetGLCamera(API::Camera& cam)
	{
		Camera::GetGLCamera(cam);
