...
namespace gE
{
    ...
    void MouseState::Update(Window& window)
    {
        for(u8 i = GLFW_MOUSE_BUTTON_1; i <= GLFW_MOUSE_BUTTON_8; i++)
            ProcessButton((MouseButton) i, _buttons[i], window.GLFWWindow());
        ...
        glfwSetInputMode(window.GLFWWindow(), GLFW_CURSOR, window.GetCursorEnabled() ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_DISABLED);
    }
}
